...
namespace
{
    ...
    HRESULT ResizePointFilter(const Image& srcImage, const Image& destImage) noexcept
    {
        ...
        XMVECTOR* row = cameraTarget + destImage.width;
        ...
        for (size_t y = 0; y < destImage.height; ++y)
        {
            ...
            if (!_StoreScanline(pDest, destImage.rowPitch, destImage.format, cameraTarget, destImage.width))
                return E_FAIL;
    ...
    HRESULT ResizeBoxFilter(const Image& srcImage, DWORD filter, const Image& destImage) noexcept
    {
        ...
        XMVECTOR* urow0 = cameraTarget + destImage.width;
        ...
        for (size_t y = 0; y < destImage.height; ++y)
        {
            ...
            if (!_StoreScanlineLinear(pDest, destImage.rowPitch, destImage.format, cameraTarget, destImage.width, filter))
                return E_FAIL;
            pDest += destImage.rowPitch;
        }
    ...
    HRESULT ResizeLinearFilter(const Image& srcImage, DWORD filter, const Image& destImage) noexcept
    {
        ...
        XMVECTOR* row0 = cameraTarget + destImage.width;
