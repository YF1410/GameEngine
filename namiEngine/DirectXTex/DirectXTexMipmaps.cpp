...
namespace
{
    ...
    HRESULT Generate2DMipsPointFilter(size_t levels, const ScratchImage& mipChain, size_t item) noexcept
    {
        ...
        XMVECTOR* cameraTarget = scanline.get();

        XMVECTOR* row = cameraTarget + width;
        ...
        for (size_t level = 1; level < levels; ++level)
        {
            ...
            for (size_t y = 0; y < nheight; ++y)
            {
                ...
                {
                    cameraTarget[x] = row[sx >> 16];
                    sx += xinc;
                }

                if (!_StoreScanline(pDest, dest->rowPitch, dest->format, cameraTarget, nwidth))
                    return E_FAIL;
                pDest += dest->rowPitch;
    ...
    HRESULT Generate2DMipsBoxFilter(size_t levels, DWORD filter, const ScratchImage& mipChain, size_t item) noexcept
    {
        ...
        XMVECTOR* urow0 = cameraTarget + width;
        XMVECTOR* urow1 = cameraTarget + width * 2;
        ...
        for (size_t level = 1; level < levels; ++level)
        {
            ...
            for (size_t y = 0; y < nheight; ++y)
            {
                ...
                for (size_t x = 0; x < nwidth; ++x)
                {
                    ...
                    AVERAGE4(cameraTarget[x], urow0[x2], urow1[x2], urow2[x2], urow3[x2])
                }

                if (!_StoreScanlineLinear(pDest, dest->rowPitch, dest->format, cameraTarget, nwidth, filter))
                    return E_FAIL;
                pDest += dest->rowPitch;
            }
    ...
    HRESULT Generate2DMipsLinearFilter(size_t levels, DWORD filter, const ScratchImage& mipChain, size_t item) noexcept
    {
        ...
        XMVECTOR* row0 = cameraTarget + width;
        XMVECTOR* row1 = cameraTarget + width * 2;
        ...
        for (size_t level = 1; level < levels; ++level)
        {
            ...
            for (size_t y = 0; y < nheight; ++y)
            {
                ...
                for (size_t x = 0; x < nwidth; ++x)
                {
                    ...
                    BILINEAR_INTERPOLATE(cameraTarget[x], toX, toY, row0, row1)
                }
    ...
    HRESULT Generate2DMipsCubicFilter(size_t levels, DWORD filter, const ScratchImage& mipChain, size_t item) noexcept
    {
        ...
        XMVECTOR* row0 = cameraTarget + width;
        XMVECTOR* row1 = cameraTarget + width * 2;
        XMVECTOR* row2 = cameraTarget + width * 3;
        XMVECTOR* row3 = cameraTarget + width * 4;
        ...
        for (size_t level = 1; level < levels; ++level)
        {
            ...
            for (size_t y = 0; y < nheight; ++y)
            {
                ...
                for (size_t x = 0; x < nwidth; ++x)
                {
                    ...
                    CUBIC_INTERPOLATE(cameraTarget[x], toY.x, C0, C1, C2, C3)
                }
    ...
    HRESULT Generate3DMipsBoxFilter(size_t depth, size_t levels, DWORD filter, const ScratchImage& mipChain) noexcept
    {
        ...
        XMVECTOR* urow0 = cameraTarget + width;
        XMVECTOR* urow1 = cameraTarget + width * 2;
        XMVECTOR* vrow0 = cameraTarget + width * 3;
        XMVECTOR* vrow1 = cameraTarget + width * 4;
                ...
                for (size_t slice = 0; slice < ndepth; ++slice)
                {
                    ...
                    for (size_t y = 0; y < nheight; ++y)
                    {
                        ...
                        for (size_t x = 0; x < nwidth; ++x)
                        {
                            ...
                            AVERAGE8(cameraTarget[x], urow0[x2], urow1[x2], urow2[x2], urow3[x2],
                ...
                for (size_t slice = 0; slice < ndepth; ++slice)
                {
                    ...
                    for (size_t y = 0; y < nheight; ++y)
                    {
                        ...
                        for (size_t x = 0; x < nwidth; ++x)
                        {
                            ...
                            TRILINEAR_INTERPOLATE(cameraTarget[x], toX, toY, toZ, urow0, urow1, vrow0, vrow1)
                        }
            ...
            else
            {
                ...
                for (size_t y = 0; y < nheight; ++y)
                {
                    ...
                    for (size_t x = 0; x < nwidth; ++x)
                    {
                        ...
                        BILINEAR_INTERPOLATE(cameraTarget[x], toX, toY, urow0, urow1)
                    }
                ...
                for (size_t slice = 0; slice < ndepth; ++slice)
                {
                    ...
                    for (size_t y = 0; y < nheight; ++y)
                    {
                        ...
                        for (size_t x = 0; x < nwidth; ++x)
                        {
                            ...
                            CUBIC_INTERPOLATE(cameraTarget[x], toZ.x, D[0], D[1], D[2], D[3])
                        }
