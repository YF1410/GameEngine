...
#define STORE_SCANLINE( type, scalev, clampzero, norm, itype, mask, row, bgr ) \
        ...
        { \
            ...
            { \
                ...
                if (pDiffusionErrors) \
                { \
                    cameraTarget = XMVectorRound(v); \
                    vError = XMVectorSubtract(v, cameraTarget); \
                ...
                { \
                    ...
                    cameraTarget = XMVectorAdd(v, ordered[ index & 3 ]); \
                    cameraTarget = XMVectorRound(cameraTarget); \
                } \
                \
                cameraTarget = XMVectorMin(scalev, cameraTarget); \
                cameraTarget = XMVectorMax((clampzero) ? g_XMZero : (XMVectorAdd(XMVectorNegate(scalev), g_XMOne)), cameraTarget); \
                \
                XMFLOAT4A tmp; \
                XMStoreFloat4A(&tmp, cameraTarget); \
                \
...
#define STORE_SCANLINE1( type, scalev, clampzero, norm, mask, row, selectw ) \
        ...
        { \
            ...
            { \
                ...
                auto dPtr = &dest[ index ]; \
                if (dPtr >= ePtr) break; \
                *dPtr = type(static_cast<type>((selectw) ? XMVectorGetW(cameraTarget) : XMVectorGetX(cameraTarget)) & mask); \
            } \
            return true; \
        } \
        return false;
    ...
    case DXGI_FORMAT_R10G10B10_XR_BIAS_A2_UNORM:
        if (size >= sizeof(XMUDEC4))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                cameraTarget = XMVectorAdd(cameraTarget, Bias);
                cameraTarget = XMVectorClamp(cameraTarget, g_XMZero, g_Scale10pc);
    ...
    case DXGI_FORMAT_D24_UNORM_S8_UINT:
        if (size >= sizeof(uint32_t))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                cameraTarget = XMVectorClamp(cameraTarget, g_XMZero, Scale2);
    ...
    case DXGI_FORMAT_B5G6R5_UNORM:
        if (size >= sizeof(XMU565))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                cameraTarget = XMVectorClamp(cameraTarget, g_XMZero, g_Scale565pc);
    ...
    case DXGI_FORMAT_B5G5R5A1_UNORM:
        if (size >= sizeof(XMU555))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                cameraTarget = XMVectorClamp(cameraTarget, g_XMZero, g_Scale5551pc);
                ...
                dPtr->z = uint16_t(static_cast<uint16_t>(tmp.z) & 0x1F);
                dPtr->w = (XMVectorGetW(cameraTarget) > threshold) ? 1u : 0u;
            }
            return true;
        }
        return false;
    ...
    case DXGI_FORMAT_B8G8R8X8_UNORM_SRGB:
        if (size >= sizeof(XMUBYTEN4))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                cameraTarget = XMVectorClamp(cameraTarget, g_XMZero, g_Scale8pc);
    ...
    case XBOX_DXGI_FORMAT_R4G4_UNORM:
        if (size >= sizeof(uint8_t))
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                cameraTarget = XMVectorClamp(cameraTarget, g_XMZero, g_Scale4pc);
