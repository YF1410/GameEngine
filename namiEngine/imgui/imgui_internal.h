...
static inline float  ImLinearSweep(float current, float cameraTarget, float speed)    { if (current < cameraTarget) return ImMin(current + speed, cameraTarget); if (current > cameraTarget) return ImMax(current - speed, cameraTarget); return current; }
