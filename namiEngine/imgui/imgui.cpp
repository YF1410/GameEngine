...
static float CalcScrollEdgeSnap(float cameraTarget, float snap_min, float snap_max, float snap_threshold, float center_ratio)
{
    if (cameraTarget <= snap_min + snap_threshold)
        return ImLerp(snap_min, cameraTarget, center_ratio);
    if (cameraTarget >= snap_max - snap_threshold)
        return ImLerp(cameraTarget, snap_max, center_ratio);
    return cameraTarget;
}
