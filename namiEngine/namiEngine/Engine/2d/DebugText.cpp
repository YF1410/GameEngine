...
void DebugText::NPrint(int len, const char* text) {
	...
	for (int i = 0; i < len; i++) {
		...
		// 座標計算
		spriteDatas[spriteIndex]->SetPosition({ this->posX + fontWidth * this->textSize * i, this->posY });
		...
		spriteDatas[spriteIndex]->SetSize({ fontWidth * this->textSize , fontHeight * this->textSize });
