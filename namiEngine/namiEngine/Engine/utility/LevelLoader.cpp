...
LevelData* LevelLoader::LoadFile(const std::string& fileName) {
	...
	for (nlohmann::json& object : deserialized["objects"]) {
		...
		if (type.compare("MESH") == 0) {
			...
			objectData.collider = object["type"].get<std::string>();
			objectData.center.m128_f32[0] = collider["collider_center"][1];
			objectData.center.m128_f32[1] = collider["collider_center"][2];
			objectData.center.m128_f32[2] = collider["collider_center"][0];
		}
