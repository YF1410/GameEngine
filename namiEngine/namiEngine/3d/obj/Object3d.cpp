...
void Object3d::Update() {
	...
	// スケール、回転、平行移動行列の計算
	matScale = XMMatrixScaling(objScale.x, objScale.y, objScale.z);
	matRot = XMMatrixIdentity();
	matRot *= XMMatrixRotationZ(XMConvertToRadians(objRotation.z));
	matRot *= XMMatrixRotationX(XMConvertToRadians(objRotation.x));
	matRot *= XMMatrixRotationY(XMConvertToRadians(objRotation.y));
	matTrans = XMMatrixTranslation(objPosition.x, objPosition.y, objPosition.z);
	...
	if (isBillboard) {
		const XMMATRIX& matBillboard = cameraObject->GetBillboardMatrix();
