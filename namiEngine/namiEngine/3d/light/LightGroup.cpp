	...
	if (SUCCEEDED(result)) {
		...
		for (int i = 0; i < DirLightNum; i++) {
			...
			if (dirLights[i].IsActive()) {
				constMap->dirLights[i].lightActive = 1;
				constMap->dirLights[i].lightv = -dirLights[i].GetLightDir();
				constMap->dirLights[i].lightColor = dirLights[i].GetLightColor();
			}
			// ライトが無効ならライト色を0に
			else {
				constMap->dirLights[i].lightActive = 0;
			}
		}
		// 点光源
		for (int i = 0; i < PointLightNum; i++) {
			...
			if (pointLights[i].IsActive()) {
				constMap->pointLights[i].lightActive = 1;
			...
			// ライトが無効ならライト色を0に
				...
				constMap->pointLights[i].lightActive = 0;
			}
		}
		// スポットライト
		for (int i = 0; i < SpotLightNum; i++) {
			...
			if (spotLights[i].IsActive()) {
				constMap->spotLights[i].lightActive = 1;
			...
			// ライトが無効ならライト色を0に
				...
				constMap->spotLights[i].lightActive = 0;
			}
		}
		// 丸影
		for (int i = 0; i < CircleShadowNum; i++) {
			...
			if (circleShadows[i].IsActive()) {
				constMap->circleShadows[i].lightActive = 1;
				...
				constMap->circleShadows[i].lightAtten = circleShadows[i].GetAtten();
				constMap->circleShadows[i].factorAngleCos = circleShadows[i].GetFactorAngleCos();
			}
			// 無効なら色を0に
			else {
				constMap->circleShadows[i].lightActive = 0;
			}
		}
...
void LightGroup::SetDirLightActive(int index, bool lightActive)
{
	assert(0 <= index && index < DirLightNum);

	dirLights[index].SetActive(lightActive);
}
...
void LightGroup::SetPointLightActive(int index, bool lightActive)
{
	assert(0 <= index && index < PointLightNum);

	pointLights[index].SetActive(lightActive);
}
...
void LightGroup::SetSpotLightActive(int index, bool lightActive)
{
	assert(0 <= index && index < SpotLightNum);

	spotLights[index].SetActive(lightActive);
}
